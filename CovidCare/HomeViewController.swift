...
class HomeViewController: UIViewController {
    ...
    override func motionEnded(_ motion: UIEvent.EventSubtype, with event: UIEvent?) {
        #if DEBUG
            guard let allowDebug = PlistHelper.getBoolFromInfoPlist(withKey: "Allow_Debug", plistName: "CovidCare-config") else {
                return
            }
    ...
    fileprivate func toggleHeaderView() {
        ...
        if (!self.allPermissionOn) {
            ...
            self.homeHeaderView.backgroundColor = UIColor.covidCareButtonColor
